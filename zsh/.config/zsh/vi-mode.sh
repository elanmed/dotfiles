...
# bindkey -sM vicmd '^F' "$fzf_cmd_prefix open $fzf_cmd_suffix"
# bindkey -sM viins '^F' "$fzf_cmd_prefix nvim $fzf_cmd_suffix"
#
# bindkey -sM vicmd '^P' 'vi-insert; file_to_edit="$(fzf)"; if [[ "$file_to_edit" != "" ]]; then; nvim "$file_to_edit"; fi \n'
# bindkey -sM viins '^P' 'vi-insert; file_to_edit="$(fzf)"; if [[ "$file_to_edit" != "" ]]; then; nvim "$file_to_edit"; fi \n'

e_fzf() {
  zle vi-insert
  eval "file="$(fzf)"; if [[ "$file" != "" ]]; then; open "$file"; fi \n"
}
zle -N e_fzf
bindkey -M vicmd '^F' e_fzf
...
bindkey -sM vicmd '^Y' 'exit\n'
bindkey -sM viins '^Y' 'exit\n'
